...
#
install_packages() {
    yum -y install zlib zlib-devel bzip2 bzip2-devel bzip2-libs openssl openssl-devel openssl-libs nfs-utils rpcbind git libicu libicu-devel make zip unzip mdadm wget gsl bc rpm-build readline-devel pam-devel libXtst.i686 libXtst.x86_64 make.x86_64 sysstat.x86_64 python-pip automake autoconf binutils.x86_64 compat-libcap1.x86_64 glibc.i686 glibc.x86_64 \
...
ubuntu_nvidia-docker() {
    ...
    curl -s -L https://nvidia.github.io/nvidia-docker/ubuntu16.04/amd64/nvidia-docker.list |
        sudo tee /etc/apt/sources.list.d/nvidia-docker.list && DEBIAN_FRONTEND=noninteractive apt-get -y update && DEBIAN_FRONTEND=noninteractive apt-get install -y nvidia-docker2 && pkill -SIGHUP dockerd && docker run --runtime=nvidia --name=digits --restart=always -d -p 5000:5000 -p 6006:6006 nvidia/digits && docker exec -ti digits bash -c "mkdir -p tensor/logs && nohup tensorboard --logdir=tensor/logs & "
}
...
centos_nvidia-docker() {
    ...
    curl -s -L https://nvidia.github.io/nvidia-docker/centos7/x86_64/nvidia-docker.repo |
        sudo tee /etc/yum.repos.d/nvidia-docker.repo && yum install -y nvidia-docker2 && pkill -SIGHUP dockerd sudo tee /etc/apt/sources.list.d/nvidia-docker.list && docker run --runtime=nvidia --name=digits --restart=always -d -p 5000:5000 -p 6006:6006 nvidia/digits && docker exec -ti digits bash -c "mkdir -p tensor/logs && nohup tensorboard --logdir=tensor/logs & "
}
...
install_docker_tf_kubectl_helm() {
    ...
    if is_master; then
        ...
        su -c "export CLOUDSDK_INSTALL_DIR=$SHARE_DATA && export CLOUDSDK_CORE_DISABLE_PROMPTS=1 && curl https://sdk.cloud.google.com | bash" $HPC_USER
...
install_slurm_config() {
    if is_master; then
        ...
        cat slurm.template.conf |
            sed 's/headcontrol/'"$MASTER_HOSTNAME"'/g' |
